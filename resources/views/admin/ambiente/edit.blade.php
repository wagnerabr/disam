...
@section('content')
<h2>Editar Ambiente <em>{{ $ambiente->nome }}</em></h2>
<div class="box-form-cadastro">
	<form method="post" action="{{ route('admin.ambiente.update', ['id' => $ambiente->id]) }}">
	    <input name="_method" type="hidden" value="PUT">
			...
			<div class="row">
			    <div class="col-xs-12 col-sm-12 col-md-12">
			        ...
			        <input class="form-control" type="text" name="legenda" value="{{ $ambiente->legenda }}" placeholder="Legenda da Imagem" required="required">
			    </div>

			    <!-- <div class="col-xs-12 col-sm-12 col-md-12">
			        <label for="cnpj">Arquivo</label>
			        <input class="form-control" type="text" name="cnpj" value="{{ $ambiente->arquivo }}" placeholder="Arquivo">
			    </div> -->
			    ...
			    <div class="col-xs-12 col-sm-12 col-md-12">
	                <div class="form-group boxTirarFoto">
	                  <h3>Enviar foto:</h3>
	                    ...
	                    <div class="box_img">
                        <img src="{{ url('/') }}/upload/ambientes/thumb/{{ $ambiente->arquivo }}">
	                    	<input type='hidden' name='arquivo' value="{{ $ambiente->arquivo }}">
	                    </div>
	                    <div class="clear"></div>
	                </div>
                </div>
  ...
  $("#file_upload").uploadifive({
    ...
    'onUploadComplete'   : function(file, data) {
      setTimeout(function() {
      	...
      	arquivo += "<input type='hidden' name='arquivo' value='"+nome+"'>";
